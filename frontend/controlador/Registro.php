      ...
      try {        
        ...
        if($datosValidos['tipo_usuario'] == Modelo_Usuario::EMPRESA){
          ...
          foreach (DIRECTORIOCORREOS as $key => $value) {
            Utils::envioCorreo($value,"Registro de empresa","Se registro una empresa. Verificar datos <br>Id de empresa: ".$id_usuario);
          }
        }
...
// $datosValidos['tipoEmpresa'] != "" -> SI LA EMPRESA ES PERSONA NATURAL
  ...
  public function google($userdata){
    $this->registroRedSocial($userdata["email"],utf8_decode($userdata['given_name']),utf8_decode($userdata['family_name']));    
  } 

  public function linkedin($userdata){
    $this->registroRedSocial($userdata["emailAddress"],utf8_decode($userdata['firstName']),utf8_decode($userdata['lastName']));    
  }

  public function twitter($userdata){    
    $this->registroRedSocial($userdata["email"],utf8_decode($userdata['name']),utf8_decode($userdata['screen_name']));    
  }

  public function facebook($userdata){
    $this->registroRedSocial($userdata["email"],utf8_decode($userdata['first_name']),utf8_decode($userdata['last_name']));    
  }
